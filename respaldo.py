...
df_unidos = df_combinado[(df_combinado['_merge_BD'] != 'both') | (df_combinado['_merge_MKT'] != 'both') | (
            df_combinado['_merge_CP'] != 'both')].drop(['_merge_BD', '_merge_MKT', '_merge_CP'], axis=1)
...
df_bd_leads['DÍAS DESDE CREACIÓN HASTA ÚLTIMO CONTACTO'] = (
            df_bd_leads['ULTIMO CONTACTO'] - df_bd_leads['FECHA DE CREACIÓn']).dt.days
...
df_bd_leads['MISMA FECHA DE INICIO Y CONTACTO'] = (df_bd_leads['DÍAS DESDE CREACIÓN HASTA ÚLTIMO CONTACTO'] == 0) & (
            df_bd_leads['DÍAS DESDE CREACIÓN HASTA HOY'] == 0)
...
correos_unicos = pd.concat(
    [df_sheet1['CORREO'], df_bd_leads['CORREO'], df_Leads_MKT['CORREO'], df_CP['CORREO']]).unique()
...
with pd.ExcelWriter(ruta_resultado, engine='openpyxl', mode='a') as writer:
    ...
    df_info_adicional = pd.merge(df_sheet1[['CORREO']],
                                 df_bd_leads_filtrado[
                                     ['NOMBRE', 'CORREO', 'ASESOR', 'ESTATUS', 'ESTADO', 'CANAL DE PROCEDENCIA',
                                      'FECHA INICIAL', 'DÍAS DESDE ASIGNACIÓN', 'DÍAS DESDE CREACIÓN',
                                      'DÍAS DESDE ÚLTIMO CONTACTO', 'ÚLTIMA FECHA DE CONTACTO', 'OCURRENCIA',
                                      'DÍAS DESDE CREACIÓN HASTA ÚLTIMO CONTACTO', 'DÍAS DESDE CREACIÓN HASTA HOY',
                                      'MISMA FECHA DE INICIO Y CONTACTO']],
                                 on='CORREO',
                                 how='left')
    ...
    df_info_adicional = pd.merge(df_sheet1[['CORREO']],
                                 df_4_ventas[
                                     ['Nombre', 'Correo', 'Telefono', 'Procedencia de venta', 'Asesor', 'Fecha Venta',
                                      'Equipo']],
                                 left_on='CORREO',
                                 right_on='Correo',
                                 how='left')
    ...
    # Ordenar por 'FECHA INICIAL' en orden descendente para cada grupo de correos
    df_info_adicional.sort_values(by=['CORREO', 'FECHA INICIAL'], ascending=[True, False], inplace=True)
